...
class Jdate {
    ...
    public function init() {
        if ($this->jalali !== null)
            $this->jalali = ($this->jalali === false) ? false : true;
        if ($this->convert !== null)
            $this->convert = ($this->convert === false) ? false : true;
        if ($this->timezone !== null)
            $this->timezone = ($this->timezone != null) ? $this->timezone : null;
    }
    ...
    function jalali_to_gregorian($jy, $jm, $jd, $mod = '') {
        $jy = $this->tr_num($jy);
        $jm = $this->tr_num($jm);
        $jd = $this->tr_num($jd); /* <= Extra :اين سطر ، جزء تابع اصلي نيست */
        $gy = ($jy <= 979) ? 621 : 1600;
        $jy-=($jy <= 979) ? 0 : 979;
